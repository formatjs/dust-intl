...
(/* istanbul ignore next */
 function (root, factory) {
    ...
    /**
     Internal iteration for `_contextGet()`.
     @protected
     @method _contextGet_iterate
     */
    function _contextGet_iterate(ctx, keys) {
        var data,
            last,
            k,
            key;
        ...
        for (k = 0; k < last; k += 1) {
            ...
            if (! data.hasOwnProperty(key)) {
                return _contextGet_iterate(ctx.tail, keys);
            }
            data = data[key];
        }
        key = keys[last];
    ...
    function _contextGet(ctx, keys) {
        return _contextGet_iterate(ctx.stack, keys);
    }

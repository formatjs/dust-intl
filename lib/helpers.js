...
(/* istanbul ignore next */
 function (root, factory) {
    ...
    function _getLocale(params, context) {
        return params.locale || context.get([CONTEXT_KEY, 'locale']) || context.get('locale') || this.locale;
    }
    ...
    function _getCurrency(params, context) {
        return params.currency || context.get([CONTEXT_KEY, 'currency']) || context.get('currency') || this.currency;
    }
    ...
    */
    ...
    /**
    Interprets `params.val` as a YRB message, number, date, or time to format.
    Or, if used as a block wrapper, stashes the `params` in the context so that
    ...
    */
    function intl(chunk, context, bodies, params) {
        ...
        // TODO -- interpret params.token
        // (there currently are no tests for params.token)

        if (!params.hasOwnProperty('val')) {
            throw new ReferenceError('A value or token must be provided.');
        }
        val = params.val;

        if (typeof val === 'number') {
            return intlNumber.call(this, chunk, context, bodies, params);
        }
        if (val instanceof Date) {
            return intlDate.call(this, chunk, context, bodies, params);
        }
        return intlMessage.call(this, chunk, context, bodies, params);
    }

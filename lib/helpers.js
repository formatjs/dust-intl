...
(/* istanbul ignore next */
 function (root, factory) {
     ...
     */
    function _getLocales(params, context) {
        return params.locales || context.get([CONTEXT_KEY, 'locales']) || this.locale;
    }
     ...
     */
    function _getFormatOptions(params, context) {
        var options;
        if (params._fmt) {
            options = context.get([CONTEXT_KEY, 'formats', params._fmt]);
            delete params._fmt;
            // TODO:  only need to copy-and-merge if there are still parameters
            options = _extend({}, options);  // shallow copy
            _extend(options, params);
        }
        else {
            options = params;
        }
    ...
    */
    function intlMessage(chunk, context, bodies, params) {
        ...
        if (params.hasOwnProperty('_msg')) {
            ...
            msg = context.get([CONTEXT_KEY, 'messages', params._key]);
        }
        else {
            throw new ReferenceError('@intlMessage needs either a `_msg` or `_key` parameter');
        }
        if (params._fmt) {
            formatOptions = context.get([CONTEXT_KEY, 'formats', params._fmt]) || {};
        }

        locales = _getLocales(params, context);
    ...
    */
    function intlNumber(chunk, context, bodies, params) {
        ...
        formatOptions = _getFormatOptions(params, context);
        locales = _getLocales(params, context);
